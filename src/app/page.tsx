...
export default function Example() {
  ...
  const redirecionar = () => {
    router.push('/login')  // Corrigi o caminho para '/login', sem a extensão .tsx
  }
            ...
            <div className="hidden sm:ml-6 sm:block">
              <div className="flex space-x-4">
                {navigation.map((item) => (
                  <a
                    key={item.name}
                    onClick={redirecionar}  // Redirecionamento aqui
                    aria-current={item.current ? 'page' : undefined}
                    className={classNames(
                      item.current ? 'bg-red-800 text-white' : 'text-white hover:bg-gray-700 hover:text-white',
